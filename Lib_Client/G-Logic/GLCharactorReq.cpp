...
//Âà¥Í¨t²Î
HRESULT GLCharacter::ReqUpRell()
{
	GLMSG::SNETPC_REQ_UPRELL NetMsg;
...
HRESULT GLCharacter::ReqInvenDrug ( WORD wPosX, WORD wPosY )
{
	...
	// ¼Õ¿¡ ¾Æ¹«°Íµµ ¾Èµé¾ú°æ¿ì »ç¿ëÇÒ¼ö ÀÖ´Â ¾ÆÀÌÅÛ
	if ( !pHold )
	{
		...
		else if (pItem->sBasicOp.emItemType == ITEM_SELFORM_BOX)
		{
		...
		else if (pItem->sBasicOp.emItemType == ITEM_UPGRADE_CARD)//µ¥¯Å¤É¯Å¥d
		{
			ReqUpgradeCard(wPosX, wPosY);
		}
...
void GLCharacter::ReqRankName(BOOL bRankName) //MMR Rank
{
	GLMSG::SNETPC_REQ_RANKNAME NetMsg;
	NetMsg.bRankName = bRankName;
	NETSENDTOFIELD(&NetMsg);
}
void GLCharacter::ReqRankMark(BOOL bRankMark) //MMR Rank
{
...
void GLCharacter::ReqStatsUpCmd(EMSTATS emStats, DWORD value)
{
	...
	if (m_wStatsPoint < value) {
		CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::RED, "¥[ÂI¥¢±Ñ! ÄÝ©ÊÂI¤£¨¬°÷.");
		return;
	}

	switch (emStats)
	{
	case EMPOW: CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREENYELLOW, "¥[ %d ¤O¶q.", value);	break;
	case EMSTR: CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREENYELLOW, "¥[ %d Åé¤O.", value);	break;
	case EMSPI: CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREENYELLOW, "¥[ %d ºë¯«.", value);	break;
	case EMDEX: CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREENYELLOW, "¥[ %d Dex.", value);	break;
	case EMSTA: CInnerInterface::GetInstance().PrintMsgText(NS_UITEXTCOLOR::GREENYELLOW, "¥[ %d Agi.", value);	break;
	}
...
HRESULT GLCharacter::SelectiveformBoxOpen(WORD wPosX, WORD wPosY)
{
	...
	SITEM* pItem = GLItemMan::GetInstance().GetItem(pInvenItem->sItemCustom.sNativeID);
	if (!pItem || pItem->sBasicOp.emItemType != ITEM_SELFORM_BOX)
	{
		CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMREQ_BOXOPEN_FB_NOBOX"));
		return E_FAIL;
	}
	...
	std::vector <ITEM::SBOX_ITEM> vecItems;

	for (int i = 0; i < ITEM::SBOX::ITEM_SIZE; i++)
	{
		SITEM* pItemInBox = GLItemMan::GetInstance().GetItem(pItem->sBox.sITEMS[i].nidITEM);

		if (pItemInBox)
		{
			vecItems.push_back(pItem->sBox.sITEMS[i]);
		}
	}
	...
	CInnerInterface::GetInstance().OpenSelectiveformBox(vecItems);
}
...
HRESULT GLCharacter::ReqSelformBoxSelect(WORD wSelItemPos)
{
	...
	GLMSG::SNET_INVEN_SELFORM_BOXOPEN NetMsg;
