...
HRESULT GLLandManClient::Create ( const char* szSetFileName, LPDIRECT3DDEVICEQ pd3dDevice, D3DXVECTOR3 &vBasicPos, SNATIVEID* pMapID, BOOL bPeaceZone )
{
	...
	//	Note : ÆÄÀÏ¿¡¼­ ÀÐÀº MAP ID °¡ ¾Æ´Ï¶ó ÀÎ¼ö·Î ³Ñ¾î¿Â ID¸¦ »ç¿ë.
	//
	...
	//	Note : ¸ãÀÇ ÃÊ±â À§Ä¡ ÁÖº¯À» ¹Ì¸® ÀÐ¾îµÐ´Ù.
	////
	...
	//	Note : Mob/Npc ÀÇ Çü»ó ÆÄÀÏµéÀ» ¹Ì¸® ÀÐ¾î µÓ´Ï´Ù.
	//
	...
	for ( ; iter!=iter_end; ++iter )
	{
		...
		if( !pCrowData )
		{
			CDebugSet::ToLogFile ( "GLLandManClient::Create()-GLCrowDataMan::GetCrowData() [%d/%d] °¡Á®¿À±â ½ÇÆÐ.", sMobID.wMainID, sMobID.wSubID );
		}
		...
		if ( pCrowData )
		{
			...
			if( !pCharData )
			{
				...
				{
					CDebugSet::ToLogFile ( "GLLandManClient::Create()-DxSkinCharDataContainer::LoadData() [%s] °¡Á®¿À±â ½ÇÆÐ.", pCrowData->GetSkinObjFile() );
				}
			}
		}
	}
...
BOOL GLLandManClient::DropItem ( SDROP_CLIENT_ITEM *pItemDrop )
{
	...
	if ( pItemDrop->fAge < 0.01f )
	{
		...
		case ITEM_UPGRADE_CARD://µ¥¯Å¤É¯Å¥d
...
BOOL GLLandManClient::DropOutChar ( DWORD dwGlobID )
{
	...
	//
	{
		...
		else
		{
			...
			//strText.freeze( false );	// Note : std::strstreamÀÇ freeze. ¾È ÇÏ¸é Leak ¹ß»ý.			
		}
	}
