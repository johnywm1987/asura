...
//	GLGaeaServer & glGaeaServer = m_pGLGaeaServer;
	...
	if (m_bPotsDelay)
	{
		m_fPotsDelay += fElapsedTime;
		if (m_fPotsDelay >= 0.17f)
		{
			m_bPotsDelay = FALSE;
			m_fPotsDelay = 0.0f;
		}
	...
	if (m_bUseActivityPointTimer)
	{
		m_fActivityPointBonusTimer += fElapsedTime;
				...
				{
					m_dwChaBuyPoint += RANPARAM::dwjifen;
					m_fActivityPointBonusTimer = 0.0f;
					...
					m_dwCombatPoints += RANPARAM::dwjifen;
					GLMSG::SNETPC_UPDATE_MONEY NetMsgMoney;
					NetMsgMoney.lnMoney = m_lnMoney;
					NetMsgMoney.dwPremium = m_dwPremiumPoints;
					NetMsgMoney.dwCombat = m_dwCombatPoints;
					GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &NetMsgMoney);
					CInnerInterface::GetInstance().PrintMsgTextDlg(NS_UITEXTCOLOR::GREEN, "你掛機我送禮,恭喜('%s')在線1小時獲得(%d)泡點", m_szName, RANPARAM::dwjifen);
					DWORD dwPointReward = RANPARAM::dwjifen;
