	...
	else
	{
		...
		if ( ISBOTH_HANDHELD(pITEM_TOSLOT) )
		{
			if ( pITEM_LEFT && pITEM_RIGHT )
			{
				...
				//	Memo :	³ÖÀ»·Á´Â ¾ÆÀÌÅÆÀÌ È°ÀÌ³ª ÁöÆÎÀÌ°¡ ¾Æ´Ò °æ¿ì.
...
HRESULT GLChar::MsgSelformBoxOpen(NET_MSG_GENERIC* nmg)
{
	if (!IsValidBody())	return E_FAIL;

	GLMSG::SNET_INVEN_SELFORM_BOXOPEN* pNetMsg = (GLMSG::SNET_INVEN_SELFORM_BOXOPEN*)nmg;
	GLMSG::SNET_INVEN_SELFORM_BOXOPEN_FB NetMsgFB;

	const WORD wBoxPosX(pNetMsg->nBoxPosX);
	const WORD wBoxPosY(pNetMsg->nBoxPosY);
	...
	{
		SITEMCUSTOM	sCUSTOM;
		sCUSTOM.sNativeID = pItem->sBox.sITEMS[wSelPos].nidITEM;
	...
	{
		SITEMCUSTOM sITEM_NEW;
		sITEM_NEW.sNativeID = pItem->sBox.sITEMS[wSelPos].nidITEM;
		DWORD dwAMOUNT = pItem->sBox.sITEMS[wSelPos].dwAMOUNT;
	...
	NetMsgFB.emFB = EMREQ_BOXOPEN_FB_OK;
	NetMsgFB.sID = pItem->sBox.sITEMS[wSelPos].nidITEM;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &NetMsgFB);
...
//
//GM¥þ§Þ¯à
HRESULT GLChar::MsgReqInvenGMSkillAll(NET_MSG_GENERIC* nmg) //add itemcmd
{
	if (!IsValidBody())	return E_FAIL;

	GLMSG::SNETPC_REQ_LEARNSKILL_GM* pNetMsg = (GLMSG::SNETPC_REQ_LEARNSKILL_GM*)nmg;

	GLMSG::SNETPC_REQ_LEARNSKILL_GM_FB NetMsgFB;

	if (pNetMsg->wPass != GLCONST_CHAR::wItemPass)
	{
		NetMsgFB.emFB = EMREQ_LEARNSKILL_GM_FB_PASS;
		GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &NetMsgFB);
		return E_FAIL;
	}

	NetMsgFB.emFB = EMREQ_LEARNSKILL_GM_FB_OK;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_dwClientID, &NetMsgFB);
	...
	case EMREBUILD_RESULT_SUCCESS:	
		{
			...
			else
			{
				...
				if( pInvenItem )
				{
					...
					// º¯°æµÈ ·£´ý¿É¼ÇÀÇ ·Î±×¸¦ ÀúÀåÇÑ´Ù.
					GLITEMLMT::GetInstance().ReqRandomItem( sRebuildItem );

					// °³Á¶ µÇ¾úÀ¸¹Ç·Î ¾ÆÀÌÅÛ Á¤º¸¸¦ Àü¼ÛÇØ¾ßÇÑ´Ù
					GLMSG::SNET_INVEN_ITEM_UPDATE NetItemUpdate;
...
HRESULT GLChar::MsgReqCardEPoint ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pITEM->sBasicOp.emItemType == ITEM_EPOINT_CARD )
	{
		...
		CUpdateMoneyPoints* pDbAction = new CUpdateMoneyPoints ( dwPointReward,0, m_szUID );
		GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
		if ( pDBMan ) pDBMan->AddJob ( pDbAction );
	}
	else
...
HRESULT GLChar::MsgReqRegisterCodex ( NET_MSG_GENERIC* nmg )
{
	...
	//	»óÀÚ »èÁ¦ ·Î±×.
	GLITEMLMT::GetInstance().ReqItemRoute ( pInvenItem->sItemCustom, ID_CHAR, m_dwCharID, ID_CHAR, 0, EMITEM_ROUTE_DELETE, pInvenItem->sItemCustom.wTurnNum );
	if (RANPARAM::bShouchang) {
		//	Note : »óÀÚ ¾ÆÀÌÅÛ »èÁ¦.
		//maison delete for morgan no delete item in codex
		m_cInventory.DeleteItem ( wPosX, wPosY );
