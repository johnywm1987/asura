...
void GLGaeaServer::PkRankAdd(DWORD dwKilled, DWORD dwKiller)
{
	...
	sPKHistory.cKF_KILLER = pCharKiller->m_nKILLFEEDSELECTOR;//maison fix kill feed style this is the fix haha, send mfeedselector data from killer to field
	...
	if (pCharKilled->m_sMapID == pCharKiller->m_sMapID)
	{
		sPKHistory.sMapID = pCharKiller->m_sMapID;
		sPKHistory.m_vKilledPos = pCharKilled->GetPosition();
		sPKHistory.m_vKillerPos = pCharKiller->GetPosition();
	}
