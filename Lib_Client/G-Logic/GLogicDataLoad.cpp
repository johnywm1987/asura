...
namespace GLCONST_CHAR
{
	BOOL	LOADFILE ( char* szFileName, BOOL bServer )
	{
		...
		if( GLOGIC::bGLOGIC_ZIPFILE ) // by °æ´ë
...
//		cFILE.getflag( "fDISPRICE", 1, 1, fDISPRICE );
		...
		cFILE.getflag("MAXPHASE", 1, 1, m_dwMasterLvl);
		cFILE.getflag("REBORNLEVEL", 1, 1, m_dwRebomLvl);
		cFILE.getflag("REBORNPOINT", 1, 1, m_dwRebomPoint);
		cFILE.getflag("REBORNSKILLPOINT", 1, 1, m_dwRebomSkillPoint);
		cFILE.getflag("REBORNMONEY", 1, 1, m_dwRebomMoney);
		cFILE.getflag("REBORN_1PAHSE", 1, 1, m_dwRebom1);

		cFILE.getflag("REBORNLEVEL_2", 1, 1, m_dwRebomLvl_2);
		cFILE.getflag("REBORNPOINT_2", 1, 1, m_dwRebomPoint_2);
		cFILE.getflag("REBORNSKILLPOINT_2", 1, 1, m_dwRebomSkillPoint_2);
		cFILE.getflag("REBORNMONEY_2", 1, 1, m_dwRebomMoney_2);
		cFILE.getflag("REBORN_2PAHSE", 1, 1, m_dwRebom1_2);

		cFILE.getflag("REBORNLEVEL_3", 1, 1, m_dwRebomLvl_3);
		cFILE.getflag("REBORNPOINT_3", 1, 1, m_dwRebomPoint_3);
		cFILE.getflag("REBORNSKILLPOINT_3", 1, 1, m_dwRebomSkillPoint_3);
		cFILE.getflag("REBORNMONEY_3", 1, 1, m_dwRebomMoney_3);
		cFILE.getflag("REBORN_3PAHSE", 1, 1, m_dwRebom1_3);
		cFILE.getflag("XIDIANMONEY", 1, 1, lnMoney1);
		cFILE.getflag("XIHONGMONEY", 1, 1, lnMoney11);
	...
	BOOL	EXP_LOADFILE ( char* szFileName )
	{
		...
		if( GLOGIC::bGLOGIC_ZIPFILE ) // by °æ´ë
			cFILE.SetZipFile( GLOGIC::strGLOGIC_SERVER_ZIPFILE ); // by °æ´ë, zipÆÄÀÏ »ç¿ë
		...
		for ( WORD i=1; i<EXPTABLE_SIZE+1; ++i )
		{
			...
			//strBuffer.freeze( false );	// Note : std::strstreamÀÇ freeze. ¾È ÇÏ¸é Leak ¹ß»ý.
		}
	...
	BOOL	LOADZBSJ(char* szFileName)
	{
		...
		if (!cFILE.open(strPath, true, GLOGIC::bGLOGIC_PACKFILE))
		{
			CDebugSet::ToLogFile("ERROR : GLCONST_CHAR::LOADZBSJ(), %s", szFileName);
			return FALSE;
		}


		cFILE.getflag("dwZbsjPartCount", 1, 1, m_dwZbsjPartCount);

		if (m_dwZbsjPartCount > (DWORD)ZBSJ_LIMIT_MAX) m_dwZbsjPartCount = (DWORD)ZBSJ_LIMIT_MAX;

		for (DWORD i = 0; i < m_dwZbsjPartCount; ++i)
		{
			CString strFLAG;
			strFLAG.Format("ZBSJ_%02d", i);
			cFILE.getflag(strFLAG.GetString(), 1, 6, dwZbsj[i].m_bType);
			cFILE.getflag(strFLAG.GetString(), 2, 6, dwZbsj[i].m_dwType);
			cFILE.getflag(strFLAG.GetString(), 3, 6, dwZbsj[i].m_fValue);
			cFILE.getflag(strFLAG.GetString(), 4, 6, dwZbsj[i].m_dwMax);
			cFILE.getflag(strFLAG.GetString(), 5, 6, dwZbsj[i].m_fPoint);
			cFILE.getflag(strFLAG.GetString(), 6, 6, dwZbsj[i].m_dwRate);//¤É¯Å¾÷²v
		...
		{
			CString strFLAGR;
			strFLAGR.Format("ZBRANDOM_%02d", i);
			cFILE.getflag(strFLAGR.GetString(), 1, 6, dwZbsjRandom[i].bLock);
			cFILE.getflag(strFLAGR.GetString(), 2, 6, dwZbsjRandom[i].emOPT);
			cFILE.getflag(strFLAGR.GetString(), 3, 6, dwZbsjRandom[i].m_fOptValue);
			cFILE.getflag(strFLAGR.GetString(), 4, 6, dwZbsjRandom[i].m_dwOptMax);
			cFILE.getflag(strFLAGR.GetString(), 5, 6, dwZbsjRandom[i].m_fOptPoint);
			cFILE.getflag(strFLAGR.GetString(), 6, 6, dwZbsjRandom[i].m_dwRate);//¤É¯Å¾÷²v
		}

		for (DWORD i = 0; i < 10; ++i)
		{
			CString strFLAGR;
			strFLAGR.Format("ZBRANDOMITEM_%02d", i);
			cFILE.getflag(strFLAGR.GetString(), 1, 2, dwZbsjRandomItem[i].sNID.wMainID);
			cFILE.getflag(strFLAGR.GetString(), 2, 2, dwZbsjRandomItem[i].sNID.wSubID);
		}
