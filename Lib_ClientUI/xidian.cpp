...
void CXidian::CreateSubControl()
{
	...
	m_pButtonReborn->CreateBaseButton("REBORN_BUTTON", CBasicTextButton::SIZE14, CBasicButton::CLICK_FLIP, ID2GAMEWORD("CHARXBOX_BUTTON", 1));
...
//	CreateUIControl("COMPETITION_CTF_PAGE_INFO_MAP");
	...
	m_pTextMapName->AddText(ID2GAMEWORD("CHARXBOX_BUTTON", 1), NS_UITEXTCOLOR::ORANGE);
	...
	if (m_pTextInfoTitle)
	{
		...
		nIndex = m_pTextInfoTitle->AddText(ID2GAMEWORD("XIDIAN_DESC", 0), NS_UITEXTCOLOR::RED);
	...
	if (m_pTextInfoGoal0)
	{
		...
		nIndex = m_pTextInfoGoal0->AddText(ID2GAMEINTEXT("MAX_XIDIAN_LEVEL"), NS_UITEXTCOLOR::GREEN);
		strCombine.Format(ID2GAMEINTEXT("MAX_XIDIAN_LEVE_REQ"), GLCONST_CHAR::m_dwMasterLvl);
	...
	if (m_pTextInfoGoal2)
	{
		...
		nIndex = m_pTextInfoGoal2->AddText(ID2GAMEINTEXT("XIDIAN_REBORN_REQUIRE"), NS_UITEXTCOLOR::GREEN);
		strCombine.Format(ID2GAMEINTEXT("MAX_XIDIAN_REBORN_REQ"));
	...
	if (m_pTextInfoMatch)
	{
		...
		nIndex = m_pTextInfoMatch->AddText(ID2GAMEINTEXT("XIDIAN_BENIFIT"), NS_UITEXTCOLOR::GREEN);
		strCombine.Format(ID2GAMEINTEXT("XIDIAN_BENIFIT_DES"), GLCONST_CHAR::m_dwRebomPoint, GLCONST_CHAR::m_dwRebomSkillPoint);
	...
	if (m_pTextInfoMatch1)
	{
		...
		int nIndex = 0;
		nIndex = m_pTextInfoMatch1->AddText(ID2GAMEINTEXT("MAX_XIDIAN_MONEY"), NS_UITEXTCOLOR::GREEN);
		strCombine.Format(ID2GAMEINTEXT("MAX_XIDIAN_MONEY_REQ"), GLCONST_CHAR::lnMoney1);
		m_pTextInfoMatch1->AddString(nIndex, strCombine.GetString(), NS_UITEXTCOLOR::WHITE);
	}
	
}

CBasicTextBox* CXidian::CreateUITextBox(const char* szControl, CD3DFontPar* pFont, int nTextAlign)
{
...
CBasicLineBox* CXidian::CreateUILineBoxQuestList(char* szBaseControl)
{
...
CBasicLineBox* CXidian::CreateUILineBoxWhiteNoBody(char* szBaseControl)
{
	CBasicLineBox* pLineBox = new CBasicLineBox;
...
CUIControl* CXidian::CreateUIControl(const char* szControl)
{
...
void CXidian::TranslateUIMessage(UIGUID ControlID, DWORD dwMsg)
{
...
void CXidian::Update(int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl)
{
