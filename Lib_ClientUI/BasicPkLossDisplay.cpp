...
{
	const DWORD& dwPkLoss = GLGaeaClient::GetInstance().GetCharacter()->m_dwPKDeath;
	...
	if (m_dwPkLossBACK < dwPkLoss)
	{
		...
		m_pPkLossText->SetOneLineText(strPkLoss, NS_UITEXTCOLOR::RED /*PLUSOPTION*/);
	}
	if (dwPkLoss == 0) // Add 0 Default Display | Eduj
	{
		m_pPkLossText->SetOneLineText("0", NS_UITEXTCOLOR::RED /*PLUSOPTION*/);
	}
