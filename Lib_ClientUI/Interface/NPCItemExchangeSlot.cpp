		...
		if ( pitem_data_req )
		{
			if ( m_pItemImageRequire[i] )
			{
				...
				if ( pitem_data_req->ISPILE() )
				{
					...
					m_pItemImageRequire[i]->SetTextDownRight( strNum.GetString(), NS_UITEXTCOLOR::RED );
				}
			}
		}	
	}
	...
	if ( pitem_data_result )
	{
		...
		if ( m_pItemImageResult )
		{
			...
			if ( pitem_data_result->ISPILE() )
			{
				...
				m_pItemImageResult->SetTextDownRight( strNum.GetString(), NS_UITEXTCOLOR::RED );
			}
		}
	}	
}
...
void CNPCItemExchangeSlot::DataReset()
{
	for ( int i=0; i<NPC_ITEM_EXCHANGE_MAX_REQ; ++i )
	{
		if ( m_pItemImageRequire[i] )
		{
			...
			m_pItemImageRequire[i]->SetTextDownRight( "", NS_UITEXTCOLOR::RED );
		}
	}
	...
	if ( m_pItemImageResult )	
	{
		...
		m_pItemImageResult->SetTextDownRight( "", NS_UITEXTCOLOR::RED );
	}
