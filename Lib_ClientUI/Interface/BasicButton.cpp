...
void CBasicButton::CreateFlipMouseOver(char* KeyValue)
{
	if (m_wFlipMode == NOT_READY)
	{
		GASSERT(0 && "CreateFlip() 함수 이후에 호출해야 합니다.");
...
void CBasicButton::SetFlip(BOOL bFlip)
{
	...
	if (m_pFlipMouseOver && bFlip)
	{
		m_pFlipMouseOver->SetVisibleSingle(FALSE);
	}
...
void CBasicButton::TranslateMouseMessage(DWORD dwMsg)
{
	...
	if (m_pMouseOver && !m_bFlip)	m_pMouseOver->SetVisibleSingle(bMouseOver);
	if (m_pFlipMouseOver && m_bFlip)	m_pFlipMouseOver->SetVisibleSingle(bMouseOver);
...
void CBasicButton::Update(int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl)
{
	...
	if (m_pFlipMouseOver) m_pFlipMouseOver->SetVisibleSingle(FALSE);
...
{
	if (!m_pMouseOver || !m_pFlipMouseOver)
	{
		GASSERT(0 && "마우스 오버 이미지가 만들어지지 않았습니다.");
		return FALSE;
	}

	return (!m_bFlip) ? m_pMouseOver->IsVisible() : m_pFlipMouseOver->IsVisible();
}
...
void CBasicButton::SetVisibleMouseOver(BOOL bVisible)
{
	...
	if (!m_bFlip) m_pMouseOver->SetVisibleSingle(bVisible);
	else
		m_pFlipMouseOver->SetVisibleSingle(bVisible);
}
