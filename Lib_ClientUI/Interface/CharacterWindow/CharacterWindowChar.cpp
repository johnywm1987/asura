...
CCharacterWindowChar::CCharacterWindowChar() :
	...
	/*Additional Info, Juver, 2017/11/05 */
	, m_pShowButton(NULL)
	, m_pHideButton(NULL)
...
void CCharacterWindowChar::CreateSubControl()
{
	...
	//pTextBox = CreateStaticControl ( "CHARACTER_WINDOW_PKSCORE_POINT_TEXT", pFont, nAlignLeft );
	//pTextBox->AddText ( ID2GAMEWORD("CHARACTER_SOCIAL_STATIC", 9) );

	//pTextBox = CreateStaticControl ( "CHARACTER_WINDOW_PKDEATH_POINT_TEXT", pFont, nAlignLeft );
	...
	m_pBadgeText = CreateStaticControl("CHARACTER_WINDOW_STAT_BADGE_VALUE", pFont, nAlignCenter);

	///*PK Score, PK Death*/
	//m_pPKScore = CreateStaticControl ( "CHARACTER_WINDOW_PKSCORE_POINT_VALUE", pFont, nAlignLeft );
	//m_pPKDeath = CreateStaticControl ( "CHARACTER_WINDOW_PKDEATH_POINT_VALUE", pFont, nAlignLeft );
	...
	{
		"RAN_INFO_CHARACTERWINDOW_SCHOOL_WINDOW_SM",
		"RAN_INFO_CHARACTERWINDOW_SCHOOL_WINDOW_HA",
		"RAN_INFO_CHARACTERWINDOW_SCHOOL_WINDOW_BH",
	};
	...
	m_pButtonLeft->CreateSub(this, "RAN_ANOTHER_CHARACTERUI_WEAR_ARROW_TURN_LEFT_DEFAULT", UI_FLAG_DEFAULT, CHARACTER_VIEW_INFO_BUTTON_LEFT);
	m_pButtonLeft->CreateFlip("RAN_ANOTHER_CHARACTER_WEARUI_ARROW_TURN_LEFT_CLICK", CBasicButton::CLICK_FLIP);
	m_pButtonLeft->CreateMouseOver("RAN_ANOTHER_CHARACTERUI_WEAR_ARROW_TURN_LEFT_OVER");
	m_pButtonLeft->SetVisibleSingle(TRUE);
	RegisterControl(m_pButtonLeft);

	m_pButtonRight = new CBasicButton;
	m_pButtonRight->CreateSub(this, "RAN_ANOTHER_CHARACTERUI_WEAR_ARROW_TURN_RIGHT_DEFAULT", UI_FLAG_DEFAULT, CHARACTER_VIEW_INFO_BUTTON_RIGHT);
	m_pButtonRight->CreateFlip("RAN_ANOTHER_CHARACTER_WEARUI_ARROW_TURN_RIGHT_CLICK", CBasicButton::CLICK_FLIP);
	m_pButtonRight->CreateMouseOver("RAN_ANOTHER_CHARACTERUI_WEAR_ARROW_TURN_RIGHT_OVER");
	m_pButtonRight->SetVisibleSingle(TRUE);
	RegisterControl(m_pButtonRight);

	m_pHideButton = CreateFlipButton("ADDIOTIONAL_WINDOW_CLOSE_BUTTON", "ADDIOTIONAL_WINDOW_CLOSE_BUTTON_F", MENU_CLOSE_BUTTON, CBasicButton::MOUSEIN_FLIP);
	m_pShowButton = CreateFlipButton("ADDIOTIONAL_WINDOW_OPEN_BUTTON", "ADDIOTIONAL_WINDOW_OPEN_BUTTON_F", MENU_OPEN_BUTTON, CBasicButton::MOUSEIN_FLIP);
...
void CCharacterWindowChar::SetAdditionalControl(BOOL bWindowOpen)
{
	PLANDMANCLIENT pLand = GLGaeaClient::GetInstance().GetActiveMap();

	if (pLand->m_bPVPTyrannyMap || pLand->m_bPVPSchoolWarsMap || pLand->m_bPVPCaptureTheFlagMap)	return;

	if (m_pHideButton)  m_pHideButton->SetVisibleSingle(bWindowOpen);
	if (m_pShowButton)  m_pShowButton->SetVisibleSingle(!bWindowOpen);
}
...
void CCharacterWindowChar::SetData(const GLCHARLOGIC& sCharData)
{
	...
	WORD wSchoolIndex = sCharData.m_wSchool;
	...
	strCombine.Format("%I64d", (LONGLONG)sCharData.m_llContributionPoint);
	m_pContrib->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITESMOKE);

	strCombine.Format("%d", (LONGLONG)sCharData.m_dwActivityPoint);
	m_pActivity->SetOneLineText(strCombine, NS_UITEXTCOLOR::WHITESMOKE);
	///*PKScore, PKDeath*/
	//strCombine.Format ( "%d", sCharData.m_dwPKScore );
	//m_pPKScore->SetOneLineText ( strCombine, NS_UITEXTCOLOR::GREENYELLOW );
	//
	//strCombine.Format ( "%d", sCharData.m_dwPKDeath );
	//m_pPKDeath->SetOneLineText ( strCombine, NS_UITEXTCOLOR::RED );
...

	{
		...
		{
			strTemp.Format("[Lvl:%d][Name:%s]", sCharData.m_wLevel, sCLUB.m_szName);
		}
		else {
...
void CCharacterWindowChar::TranslateUIMessage(UIGUID ControlID, DWORD dwMsg)
{
	...
	case MENU_OPEN_BUTTON:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			if (CHECK_LB_UP_LIKE(dwMsg))
			{
				CInnerInterface::GetInstance().SetCharacterWindowAdditionalOpen(TRUE);
			}
		}
	}
	break;
	...
	case MENU_CLOSE_BUTTON:
	{
		if (CHECK_MOUSE_IN(dwMsg))
		{
			if (CHECK_LB_UP_LIKE(dwMsg))
			{
				CInnerInterface::GetInstance().SetCharacterWindowAdditionalOpen(FALSE);
			}
		}
	}
	break;
...
void CCharacterWindowChar::Update(int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl)
{
	...
	SetAdditionalControl(CInnerInterface::GetInstance().IsVisibleGroup(CHARACTER_ADDITIONAL_WINDOW));
}
