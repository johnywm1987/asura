	...
	if ( found!=m_listDISP_NAME.end() )
	{
		...
		if ( pDISP->DIFFERENT(sDISP_NAME) )
		{
			...
			//	Note : 새로 생성.
			{
				...
				bool bGM = ( sDISP_NAME.m_wUserlvl > 1 );

				/*activity system, Juver, 2017/11/05 */
				/*pvp capture the flag, Juver, 2018/01/31 */
				if( sDISP_NAME.m_szBADGE[0] != _T('\0') )
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_bRankName,sDISP_NAME.m_bRankMark, sDISP_NAME.m_szBADGE, sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam,sDISP_NAME.m_dwMMR, bGM );
				else
					pDISP_NEW->SetName ( sDISP_NAME.m_szNAME,sDISP_NAME.m_bRankName,sDISP_NAME.m_bRankMark, "", sDISP_NAME.m_dwCOLOR, sDISP_NAME.m_wSCHOOL, sDISP_NAME.m_emPARTY, sDISP_NAME.m_wCaptureTheFlagTeam,sDISP_NAME.m_dwMMR, bGM );
