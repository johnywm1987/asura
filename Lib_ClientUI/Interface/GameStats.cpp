...
void CGameStats::CreateSubControl ()
{
	...
	m_pPaodianText = new CBasicTextBox;
	m_pPaodianText->CreateSub(this, "GAMESTATS_PAODIAN");
	m_pPaodianText->SetFont(pFont);
	m_pPaodianText->SetTextAlign(TEXT_ALIGN_CENTER_X);
	RegisterControl(m_pPaodianText);
...
void CGameStats::UpdateInfo()
{
	...
	if ( m_pPingText )
	{
		...
		if (dwUSERLVL > 1)
		{
			...
			CString strPing("");
			strPing.Format("¯u¹ê:%2.0f ms", fPing);
