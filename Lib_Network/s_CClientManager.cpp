...
int CClientManager::SetAcceptedClient(DWORD dwClient, SOCKET sSocket)
{
	...
	else
	{
		CConsoleMessage::GetInstance()->Write(
			_T("«È¤áºÝ³s±µ¥¢±Ñ (Client ID:%d) (%s:%d)"),
			dwClient,
			pData->GetIP(),
			pData->GetPort() );
		return NET_ERROR;
...
void CClientManager::IPSetClientMax( WORD _wMAX, bool bEnableBlock )
{
	...
	if ( m_wClientIPMax == 0 ){
		CConsoleMessage::GetInstance()->Write( _T( "¼ÈµL­­¨î¦h¶}"), m_wClientIPMax );
	}
	else{
		CConsoleMessage::GetInstance()->Write( _T( "¥Ø«e¥i%d¦h¶}"), m_wClientIPMax );
	}
}
...
void CClientManager::IPRemove( std::string _strIP )
{
	...
	if ( iterip != m_mapClientIP.end() ){
		...
		CConsoleMessage::GetInstance()->Write(_T("IPLimit ²¾°£ IP:%s"), _strIP.c_str());
	}
...
void CClientManager::IPClear()
{
	...
	m_mapClientIP.clear();
	m_llDenyNum = 0;
	CConsoleMessage::GetInstance()->Write(_T("IPLimit§ó·s¦¨¥\ Clear all ip"));
	LockOff();
}
