...
{	
	...
	/////////////////////////////////////////////////////////////////////////////
		{
			//	Note : ¸¸¾à (dwMAX_CLIENT) º¸´Ù Å« ID ¶ó¸é ÇÊµå ¼­¹ö¿¡¼­ µµÂøÇÏ¿´À½.
			//		ÀÌ ID¸¦ Å¬¶óÀÌ¾ðÆ® ID·Î º¯È¯.
...
void CAgentServer::OtherMsgProcess(MSG_LIST* pMsg)
{
	...
	default :
		CConsoleMessage::GetInstance()->Write( _T("«È¤áºÝ©MªA°ÈºÝ¤£¤@­P ( %d ) ºÝ¤f ID ( %d )"), nmg->nType, pMsg->dwClient );		
		break;
	};
}
...
void CAgentServer::MsgGameJoin(MSG_LIST* pMsg)
{
	...
	// nProtect ¼­¹ö ÀÎÁõÀ» »ç¿ëÇÑ´Ù¸é Ä³¸¯ÅÍ°¡ Á¢¼ÓÇÒ¶§ ÀÎÁõÀÌ ¿Ô´ÂÁö Ã¼Å©ÇÑ´Ù.	
	...
	// ÀÎÁõÀ» Åë°úÇÏÁö ¾ÊÀº »óÅÂ¿¡¼­ Ä³¸¯ÅÍ Á¤º¸¿äÃ»ÇÔ
	...
	//	Note : db action¿¡ ÄÉ¸¯ÅÍ Á¤º¸ °¡Á®¿À±â ¿¹¾à, ÀÌÈÄ ÄÉ¸¯ÅÍ Á¢¼Ó.
	//
...
void CAgentServer::MsgAgentReqJoin(MSG_LIST* pMsg)
{
	...
	// ÇöÀç Áß±¹ÀÌ ¾Æ´Ï´õ¶óµµ ÀÌº¥Æ® ½Ã°£À» À§ÇØ ÇöÀç ·Î±×ÀÎÇÑ ½Ã°£À» ±¸ÇÑ´Ù.
	// ÇöÀç ·Î±×ÀÎÇÑ ½Ã°£
	CTime crtTime			   = CTime::GetCurrentTime();
	sChinaTime.loginTime	   = crtTime.GetTime();

	//////////////////////////////////////////////////////////////////////////
	// Áß±¹ Å½´Ð ¹æ½Ã ½Ã½ºÅÛ °è»ê
	...
	if( sChinaTime.lastOffLineTime != 0 )
	{
		...
		// ·Î±×¿ÀÇÁÇÑ ½Ã°£ÀÌ 5½Ã°£º¸´Ù Å©°Å³ª °°À¸¸é ´©Àû½Ã°£À» ÃÊ±âÈ­ÇÑ´Ù.
		...
		//if( totalOffTime.GetTotalMinutes() >= 3 )		
		{
			...
			{
				// Áß±¹ ·Î±×ÀÎ½Ã ÇÃ·¹ÀÌ ´©Àû½Ã°£ÀÌ 5½Ã°£ÀÌ ³Ñ°í ¿ÀÇÁ¶óÀÎ ´©Àû½Ã°£ÀÌ 5½Ã°£ÀÌ µÇÁö ¾Ê¾ÒÀ» °æ¿ì ¿¡·¯ ¸Þ½ÃÁö¸¦ º¸³½´Ù.
...
//			CConsoleMessage::GetInstance()->Write( _T("INFO:Vietnam GameTime not correct value...") );
	...
	// User DB ¿¡ ÇÃ·¹ÀÌ ÁßÀÎ Ä³¸¯ÅÍ ÀÌ¸§À» ¼¼ÆÃÇÑ´Ù.
	if( pChar != NULL )
	{	
...
///////////////////////////////////////////////////////////////////////////////
// Agent->Client
// Å¬¶óÀÌ¾ðÆ®°¡ Ä³¸¯ÅÍ ¼±ÅÃ È­¸é¿¡¼­ µð½ºÇÃ·¹ÀÌ ÇØ¾ßÇÒ
// Ä³¸¯ÅÍÀÇ °íÀ¯¹øÈ£¸¦ Àü¼ÛÇÑ´Ù.
...
//	int nChannel;
	...
	// ÇØ´ç Ã¤³ÎÀ» Á¶»çÇØ¼­ full ÀÎÁö °Ë»çÇØ¾ß ÇÑ´Ù.
	if (IsChannelFull(nChannel) == true)
	{
		// ÇØ´ç Ã¤³ÎÀÌ full ÀÌ¸é ¿¬°áÀ» ²÷¾î¾ß ÇÑ´Ù.
		// Full ÀÏ °æ¿ì Ã³¸®ÇÑ´Ù.		
		CloseClient(dwClient);
		CConsoleMessage::GetInstance()->Write(
											  _T("CH(%d) is FULL"),
...
{
	// ÇØÅ·À» ÅëÇØ¼­ nChaNum À» 0 À¸·Î ¸¸µé¼ö ÀÖÀ½	
	if (nChaNum <= 0) 
	{
		return;
	}
...
void CAgentServer::MsgCreateCharacter( MSG_LIST* pMsg )
{
	...
	int				nUesrNum		= 0;
	int				nIndex			= 0;
	int				nExtremeM		= 0; // ±Ø°­ºÎ ³²ÀÚ »ý¼º °¹¼ö
	int				nExtremeW		= 0; // ±Ø°­ºÎ ¿©ÀÚ »ý¼º °¹¼ö
	int				nExtremeCheck	= 0; // ±Ø°­ºÎ »ý¼º °¡´É Ã¼Å©º¯¼ö
	int				nSvrGrp			= 0;
	int				nChaNum			= 0;
    WORD            wChaRemain		= 0;
	...
	// Ä³¸¯ÅÍ ÀÎµ¦½º ¿À·ù, Ä³¸¯ÅÍ ¸¸µé ¼ö ¾øÀ½.
	if (nIndex < 0 || nIndex >= GLCI_NUM_8CLASS )	
	{
		nChaNum = -1; // Ä³¸¯ÅÍ ¿¡·¯
		nncfd.nResult = EM_CHA_NEW_FB_SUB_LENGTH; // ±ÛÀÚ¼ö Á¦ÇÑ
		SendClient(dwClient, &nncfd);
		return;
	}	
	...
	{
		nChaNum = -1; // Ä³¸¯ÅÍÀÌ¸§ ¿¡·¯
		nncfd.nResult = EM_CHA_NEW_FB_SUB_BAD_NAME; // ±ÛÀÚ¼ö Á¦ÇÑ
		SendClient(dwClient, &nncfd);
		return;
	}
	...
	{
		nChaNum = -1; // Ä³¸¯ÅÍÀÌ¸§ ¿¡·¯
		nncfd.nResult = EM_CHA_NEW_FB_SUB_THAICHAR_ERROR; // ±ÛÀÚ¼ö Á¦ÇÑ
		SendClient(dwClient, &nncfd);
		return;
	}
#endif
	...
	{
		nChaNum = -1; // Ä³¸¯ÅÍÀÌ¸§ ¿¡·¯
		nncfd.nResult = EM_CHA_NEW_FB_SUB_VNCHAR_ERROR; // ±ÛÀÚ¼ö Á¦ÇÑ
		SendClient(dwClient, &nncfd);
		return;
	}
	...
	{
		// ¼­¹ö¿¡ ÀúÀåµÈ ±Ø°­ºÎ »ý¼º Ã¼Å© º¯¼ö ¹Þ¾Æ¿À±â	
		nExtremeM = m_pClientManager->GetExtremeM( dwClient );

		// ±Ø°­ºÎ ³²ÀÚ »ý¼ºÀ» ºÒ°¡ÇÒ °æ¿ì, nnc->wSex(¿©ÀÚ) = 0
		if( 0 >= nExtremeM )
		{
			nncfd.nResult = EM_CHA_NEW_FB_SUB_MAX;
			SendClient(dwClient, &nncfd);
	...
	else if( GLCI_EXTREME_W == nIndex )
	{
		...
		// ±Ø°­ºÎ ¿©ÀÚ »ý¼ºÀÌ ºÒ°¡ÇÒ °æ¿ì, nnc->wSex(³²ÀÚ) = 1
		if( 0 >= nExtremeW )
		{
			nncfd.nResult = EM_CHA_NEW_FB_SUB_MAX;
			SendClient(dwClient, &nncfd);
...
{	
	...
	if (nChaNum > 0) // Ä³¸¯ÅÍ »ý¼º ¼º°ø
	{
		nncfd.nChaNum = nChaNum;
		nncfd.nResult = EM_CHA_NEW_FB_SUB_OK;

		// ·ÎÁ÷»óÀÇ ±Ø°­ºÎ ³²/¿© »ý¼º °¡´É ¼ýÀÚ¸¦ ¹Þ¾Æ¿Â´Ù.
		nExtremeM = m_pClientManager->GetExtremeM( dwClient );
		nExtremeW = m_pClientManager->GetExtremeW( dwClient );

		// DB¿¬»ê»óÀÇ ±Ø°­ºÎ ³²/¿© »ý¼º °¡´É ¼ýÀÚ¸¦ ¹Þ¾Æ¿Â´Ù.
	...
	else
	{
		...
		if (nChaNum == DB_ERROR) // Ä³¸¯ÅÍ »ý¼º(DB) ¿¡·¯¹ß»ý...
		{    
		...
		else if (nChaNum == DB_CHA_MAX) // ¸¸µé¼ö ÀÖ´Â Ä³¸¯ÅÍ ¼ýÀÚ ÃÊ°ú
		{
			if (m_bTestServer) wChaRemain = m_pClientManager->GetChaTestRemain(dwClient);
			else               wChaRemain = m_pClientManager->GetChaRemain(dwClient);
			nncfd.wChaRemain = wChaRemain;
		...
		else if (nChaNum == DB_CHA_DUF) // °°Àº ÀÌ¸§À» °¡Áø Ä³¸¯ÅÍ Á¸Àç
		{
			if (m_bTestServer) wChaRemain = m_pClientManager->GetChaTestRemain(dwClient);
			else               wChaRemain = m_pClientManager->GetChaRemain(dwClient);
			nncfd.wChaRemain = wChaRemain;
...
///////////////////////////////////////////////////////////////////////////////
// ¿©¸§¹æÇÐ PC ÇÁ·Î¸ð¼Ç ¾ÆÀÌÅÛ 'ÆãÆã' ÀÌº¥Æ®
// PC ¹æ¿¡¼­ ÇÏ·ç¿¡ ÇÑ»ç¶÷ÀÌ ÇÑ¹ø¸¸ °¡´É
// nResult
// 1 : ÀÌº¥Æ® ÀÀ¸ð°¡´É
// 2 : ÀÌ¹Ì ÀÌº¥Æ®¿¡ ÀÀ¸ðÇßÀ½, ÇÏ·ç¿¡ ÇÑ¹ø¸¸ °¡´É.
// 3 : ÀÀ¸ðºÒ°¡ PC ¹æ IP ¾Æ´Ô
void CAgentServer::MsgLogPungPungCheckBack(int nResult, DWORD dwClient, const char* szUserIP, USHORT uPort)
{
	...
	if ((strcmp(szUserIP, m_pClientManager->GetClientIP(dwClient)) == 0) &&
		(m_pClientManager->GetClientPort(dwClient) == uPort))
	{
        int nPrize = 0; // ´çÃ·µÈ »óÇ°.        
        // 1 ¹ø ÀÀ¸ð°¡´ÉÀÏ¶§¸¸ ¾ÆÀÌÅÛ ´çÃ·À»...
        if (nResult == 1)
        {
            // ¾ÆÀÌÅÛ ´çÃ·Ã³¸®
