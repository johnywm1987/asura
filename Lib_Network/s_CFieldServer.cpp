...
int CFieldServer::Start()
{
	...
	// nProtect GameGuard dll À» ÃÊ±âÈ­ ÇÑ´Ù.
	...
	// ¾Ë°í ÀÖ´Â ÁÖ¼Ò¿¡ ÀÚ½ÅÀÇ ÁÖ¼Ò, Session ¼­¹öÀÇ ÁÖ¼Ò, Agent ¼­¹öÀÇ ÁÖ¼Ò¸¦ µî·ÏÇÑ´Ù.
...
int CFieldServer::Stop()
{
	...
	CConsoleMessage::GetInstance()->WriteFile(_T("== FB¡G¶Ã¨pªA¤@±øÀsªA°È¾¹¥¿¦b¦Û°ÊÃö³¬¡I½Ð¤Å±j¨îÃö³¬¡I "));
...
{	
	...
	if ( dwClient < NET_RESERVED_SLOT )
	{
		if (dwClient == m_dwSession && m_bIsRunning == true)
		{
			...
			{
				// ¿¬°áÀÌ ²÷°åÀ» °æ¿ì ¹Ù·Î ´Ù½Ã ¿¬°á ½Ãµµ ÇÏÁö ¾Ê´Â´Ù. Heart Beat¿¡¼­ ´Ù½Ã Ã¼Å©ÇÏ±â ¶§¹®ÀÌ´Ù.
...
{	
	LockOn();
	{
		...
		{
			CConsoleMessage::GetInstance()->Write( _T("Session Server ¤w³QÃö³¬ %d"), dwClient );
			...
			if (dwClient == NET_RESERVED_SESSION && m_bIsRunning == true)
			{
				...
				CConsoleMessage::GetInstance()->Write(_T("================================"));
				CConsoleMessage::GetInstance()->Write(_T("Session ³s±µ¥¢±Ñ"));
				CConsoleMessage::GetInstance()->Write(_T("================================"));
				LockOff();
				return;
			}
		...
		{	
			...
			if (dwFieldAgent != 0)
			{
				...
				{	
					...
					{
						CConsoleMessage::GetInstance()->Write(LOG_CONSOLE, 
							_T("(CID %d, GID %d) ª±®a±j¨î¤U½u,¤w¦w¥þ«O¦sª±®a¼Æ¾Ú"),
							dwFieldAgent,
							dwGaeaID);
						// °¡ÀÌ¾Æ¼­¹ö Ä³¸¯ÅÍ µå¶ø ¿¹¾à
						GLGaeaServer::GetInstance().ReserveDropOutPC(dwGaeaID);
...
{
	CConsoleMessage::GetInstance()->Write( _T("«È¤áºÝ¤£¤Ç°t (Send)(%d)¥N½X"), dwClient);
}
...
void CFieldServer::SendAllClient(LPVOID pBuffer)
{
	...
	CConsoleMessage::GetInstance()->Write(_T("µo°e«H®§¨ì¥þªAª±®a"));
...
void CFieldServer::RecvMsgProcess()
{
	...
	while (pMsg != NULL)
	{
		// ¸Þ½ÃÁö Ã³¸®
		MsgProcess(pMsg);					
		pMsg = m_pRecvMsgManager->GetMsg(); // ´ÙÀ½¸Þ½ÃÁö °¡Á®¿À±â
	}
...
void CFieldServer::FieldSrvHeartBeatCheck( DWORD dwUdpateTime )
{
	...
	{	
		// Àß¸øµÈ Á¢¼Ó Á¾·á
		// Á¢¼Ó¸¸ ÇÏ°í ´õÀÌ»ó ¸Þ½ÃÁö¸¦ º¸³»Áö ¾Ê¾Æ¼­ GAEAID_NULL ÀÎ »óÅÂ
...
void CFieldServer::PrintDebugMsg( DWORD dwUdpateTime )
{
	...
	{		
		...
		// Ã³¸®½Ã°£ÀÌ 500msec ÀÌ»óÀÌ¶ó¸é ±â·ÏÇÑ´Ù.
		if (dwProfileEnd > 500)
		{
			CConsoleMessage::GetInstance()->Write(
