...
void CIPFilter::AddIPBlock( std::string _strIP, bool _bBroadcast /*= false*/, bool _bFromFile )
{
	if ( _strIP.size() <= 0 )	
	{
		...
		m_pConsole->WriteFile(_T("偵測到 IP(%s) 為人為IP,有可能是攻擊"), _strIP.c_str());
	...
	if ( iter == m_setIPBlock.end() )	
	{
		...
		//	m_pConsole->WriteFile(_T( "Block IP(%s)"), _strIP.c_str() );
			m_pConsole->WriteFile(_T("封鎖該IP(%s)封包攻擊！"), _strIP.c_str());
			CDebugSet::ToFileWithTime("IP封包攻擊被封記錄.txt", ("該IP(%s)被封鎖到防火墻！"), _strIP.c_str());
	...
	if ( m_pConsole )
		//m_pConsole->WriteFile(_T( "IPFilter IP(%s) Added To BlockList"), _strIP.c_str() );
			m_pConsole->WriteFile(_T("該IP(%s)被封鎖到防火墻！"), _strIP.c_str());
			CDebugSet::ToFileWithTime("IP被封到防火墻記錄.txt", ("該IP(%s)被封鎖到防火墻！"), _strIP.c_str());
...
void CIPFilter::AddIPKnown( std::string _strIP, bool _bBroadcast /*= false*/ )
{
	...
	m_pConsole->WriteFile(_T(" IP(%s) 添加到白名單"), _strIP.c_str());
...
void CIPFilter::ListLoad()
{
	...
	//	m_pConsole->WriteFile(_T( "IPFilter LoadList Known(%d) Blocked(%d) "), GetIPKnownNum(), GetIPBlockNum() );
		m_pConsole->WriteFile(_T("IPFilter 總白名單IP(%d) 黑名單IP(%d) "), GetIPKnownNum(), GetIPBlockNum());
}
