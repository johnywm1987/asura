...
///////////////////////////////////////////////////////////////////////////////
// TEST Session ¼­¹ö : Ä³¸¯ÅÍ »ý¼º¼ýÀÚ 1 °¨¼Ò
...
///////////////////////////////////////////////////////////////////////////////
// Session : »ý¼ºÇÒ ¼ö ÀÖ´Â Ä³¸¯ÅÍ ¼ö 1 Áõ°¡
...
///////////////////////////////////////////////////////////////////////////////
// Agent : »ý¼ºÇÒ ¼ö ÀÖ´Â Ä³¸¯ÅÍ ¼ö 1 Áõ°¡
...
///////////////////////////////////////////////////////////////////////////////
// TEST Session : »ý¼ºÇÒ ¼ö ÀÖ´Â Ä³¸¯ÅÍ ¼ö 1 Áõ°¡
...
///////////////////////////////////////////////////////////////////////////////
// TEST Agent : »ý¼ºÇÒ ¼ö ÀÖ´Â Ä³¸¯ÅÍ ¼ö 1 Áõ°¡
...
int CAgentThaiUserCheck::Execute (CServer* pServer)
{
	...
	switch (nResult)
	{
	case DB_ERROR : // ½Ã½ºÅÛ¿¡·¯·Î ·Î±×ÀÎ ½ÇÆÐ (-1)
		NetMsgFB.nResult = EM_LOGIN_FB_SUB_SYSTEM;
		break;
	case 0 : // ID / PWD ºÒÀÏÄ¡ (»õ·Î¿î °èÁ¤À» »ý¼º ¶Ç´Â ÀçÀÔ·Â¿ä±¸)	
		NetMsgFB.nResult = EM_LOGIN_FB_SUB_INCORRECT;
		break;
	case 1 :
	case 2 : // ID/PWD °¡ ÀÏÄ¡ÇÏ°í, IP °¡ »ç¿ë°¡´É	
	case 3 : // ID/PWD °¡ ÀÏÄ¡ÇÏ°í, IP Á¤º¸´Â ¾øÀ½ (ÀÏ¹ÝÀûÀ¸·Î ·Î±×ÀÎ ¼º°ø)
		// »ç¿ëÀÚ Á¤º¸¸¦ °¡Á®¿Â´Ù.
		nRetCode = COdbcManager::GetInstance()->ThaiGetUserInfo( m_strUserID.GetString(), &NetMsgFB );
		if (nRetCode == DB_ERROR) // °¡Á®¿À±â ½ÇÆÐ
		{
			NetMsgFB.nResult = EM_LOGIN_FB_SUB_SYSTEM;
			// ·Î±×¾Æ¿ô Ã³¸®
		...
		break;
	case 4 : // ID/PWD °¡ ÀÏÄ¡ÇÏÁö¸¸ IP °¡ »ç¿ëºÒ°¡´É »óÅÂ(Á¢±Ù Â÷´ÜµÈ IP)
...
int CAgentUserCheck::Execute(CServer* pServer)
{
	...
	// ³»ºÎ ¼­¹ö¿¡¼­ ÅÂ±¹ User Class Type Ã¼Å©¸¦ À§ÇØ¼­ Ãß°¡
//	int nResult1 = COdbcManager::GetInstance()->ThaiUserClassCheck( m_strUserIP.GetString() );
/*
	// ¸»·¹ÀÌ½Ã¾Æ PC¹æ ÀÌº¥Æ®
...
//	NetMsgFB.nThaiCC_Class = nResult1;
	...
	case DB_ERROR : // ½Ã½ºÅÛ¿¡·¯·Î ·Î±×ÀÎ ½ÇÆÐ (-1)
		NetMsgFB.nResult = EM_LOGIN_FB_SUB_SYSTEM;
		CConsoleMessage::GetInstance()->Write(" AGENTªA°ÈºÝÀË¬dµ²ªG %d ¡]¼Æ¾Ú®w¿ù»~¡^", nResult);
		break;
	case 0 : // ID / PWD ºÒÀÏÄ¡ (»õ·Î¿î °èÁ¤À» »ý¼º ¶Ç´Â ÀçÀÔ·Â¿ä±¸)	
		NetMsgFB.nResult = EM_LOGIN_FB_SUB_INCORRECT;
		CConsoleMessage::GetInstance()->Write("AGENTªA°ÈºÝÀË¬dµ²ªG %d" , nResult);
		break;
	case 1 :
	case 2 : // ID/PWD °¡ ÀÏÄ¡ÇÏ°í, IP °¡ »ç¿ë°¡´É	
	case 3 : // ID/PWD °¡ ÀÏÄ¡ÇÏ°í, IP Á¤º¸´Â ¾øÀ½ (ÀÏ¹ÝÀûÀ¸·Î ·Î±×ÀÎ ¼º°ø)
				                         ...
				                         DWORD dwClientNum)
{
	...
	m_nSvrNum			= nSvrNum;		
	m_dwClient			= dwClient; 
	m_dwClientNum		= dwClientNum; // Agent Å¬¶óÀÌ¾ðÆ® ¹øÈ£
}
...
int CAgentDaumUserCheck::Execute(CServer* pServer)
{
	...
	case 6 : // Block µÈ »óÅÂ
		...
		CConsoleMessage::GetInstance()->Write("CAgentDaumUserCheck result %d", nResult);
		break;
	case 7 : // À¯Àú°¡ Á¸ÀçÇÕ´Ï´Ù. À¯Àú »ðÀÔ½ÇÆÐ
	default : // ´Ù¸¥ ¸ðµç°æ¿ì ·Î±×ÀÎ ½ÇÆÐ
...
int CAgentDaumUserCheck::Execute( CServer *pServer )
{
	...
	case 6 : // Block µÈ »óÅÂ
		...
		CConsoleMessage::GetInstance()->Write("CAgentDaumUserCheck result %d", nResult);
		break;
	case 20 : // Web ÀÎÁõ ½Ã°£ ÃÊ°ú 
		nlfd2.nResult = EM_LOGIN_FB_SUB_EXPIRED;
		CConsoleMessage::GetInstance()->Write("CAgentDaumUserCheck result %d", nResult);
		break;
	case 21 : // UUID°¡ webÁßº¹ ·Î±×ÀÎÀ¸·Î ÀÎÇØ »ç¶óÁü
		...
		DWORD dwClientNum )
{
	...
	m_nSvrGrp = nSvrGrp;
	m_nSvrNum = nSvrNum;
	m_dwClientNum = dwClientNum; // Agent ¿¡¼­ÀÇ Client ¹øÈ£
}
...
int CAgentTerraPassCheck::Execute(CServer* pServer)
{
	...
	switch(nResult)
	{
	case DB_ERROR : // DB¿¡·¯·Î ÆÐ½º¿öµå ¾÷µ¥ÀÌÆ® ½ÇÆÐ (-1)
		nlfd2.nResult = DB_ERROR;
	...
	case 1 : // ID / PWD ºÒÀÏÄ¡ (·Î±×ÀÎ ½ÇÆÐ) (1)
		...
		CConsoleMessage::GetInstance()->Write(_T("CAgentTerraUserCheck result %d"), nResult);
		break;
	case 2 : // ÃÊ±â ÆÐ½º¿öµå ¾÷µ¥ÀÌÆ® ¼º°ø
		nlfd2.nResult = EM_LOGIN_FB_SUB_PASS_OK;
		break;			
	default : // ´Ù¸¥ ¸ðµç°æ¿ì ·Î±×ÀÎ ½ÇÆÐ(1)
		nlfd2.nResult = DB_ERROR;
...
{	
	...
	case 5 : // Áßº¹Á¢¼Ó
		nlfd2.nResult = EM_LOGIN_FB_SUB_DUP;
		// Áßº¹µÈ Á¢¼ÓÀÌ¶óµµ À¯Àú Á¤º¸¸¦ °¡Á®¿Í¼­ ¼ÂÆÃÇÑ´Ù.
		COdbcManager::GetInstance()->ExciteGetUserInfo(m_strUserID.GetString(), &nlfd2);
		CConsoleMessage::GetInstance()->Write(_T("CAgentExciteUserCheck result %d"), nResult);
		break;
	case 6 : // Block µÈ »óÅÂ
											 ...
											 int nUserNum)
{
	...
	m_dwClientNum = dwClientNum; // Agent ¿¡¼­ÀÇ Client ¹øÈ£
	m_nUserNum = nUserNum;		// WhiteRock System¿¡¼­ÀÇ UserNum
}
...
int CAgentExcitePassCheck::Execute(CServer* pServer)
{
	...
	case 2 : // ÃÊ±â ÆÐ½º¿öµå ¾÷µ¥ÀÌÆ® ¼º°ø
		nlfd2.nResult = EM_LOGIN_FB_SUB_PASS_OK;
		break;			
	case 3 : // ÃÊ±â ÆÐ½º¿öµå ¾÷µ¥ÀÌÆ® ¼º°ø
