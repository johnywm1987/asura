...
//쭌Ν쮝꽝
int COdbcManager::SetChaRebornChange(DWORD dwChaNum, int nReborn, int nLevel, int nStatsPoint, int nSkillPoint, int nlnMoney, int nlnVoteP, int nlnPremP)
{
	if (nReborn < 0 || nLevel < 0 || nStatsPoint < 0 || nSkillPoint < 0 || nlnMoney < 0 || nlnVoteP < 0 || nlnPremP < 0)
		return DB_ERROR;

	TCHAR szTemp[128] = { 0 };
	_snprintf(szTemp, 128, "{call sp_UpdateChaRebornChange(%u,%d,%u,%u,%I64d,%I64d,%I64d,?)}", dwChaNum, nReborn, nLevel, nStatsPoint, nSkillPoint, nlnMoney, nlnVoteP, nlnPremP);//maison check
